...
function Contact(){
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_3suz5b8', 'template_lctllas', form.current, 'DjTFzTKBO3anEGS1j')
      .then((result) => {
          console.log('Success!', result.status, result.text);
          alert("메일을 성공적으로 보냈습니다.")
      }, (error) => {
        console.log('Failed...', error);
        alert("메일을 전송하는데 실패했습니다.\n"+"livingsnow@gmail.com 으로 메일을 보내주세요");
      });
  };
  ...
  return (
    <section id="contact" className="section">
      ...
      <div className="section__wrapper contact__wrapper">
        <form ref={form} onSubmit={sendEmail} id="contact-form" className="contact__form">
          <input type="text" className="input" name="user_name" id="userName" placeholder=" 성함 (필수)" />             
          <input type="text" className="input"  id="phone" placeholder=" 연락처 (생략 가능)" /><br/>
          <input type="text" className="input"name="user_email"  id="userEmail" placeholder=" 보내주시는 분의 메일 주소를 입력해주세요 (필수)" /><br/>
          <textarea id="message" name="message" className="input" rows="5" placeholder=" 내용을 입력해주세요 (필수)"></textarea><br/>
          <input type="submit" id="btn" className="contact__form-sendBtn" value="Send"/>
        </form>
      </div>
    </section>
  )
